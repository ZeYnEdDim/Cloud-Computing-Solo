...
\begin{itemize}
    \item \textbf{1h}: one hourly file;
    \item \textbf{4h}: four hourly files;
    \item \textbf{8h}: eight hourly files;
    \item \textbf{16h}: sixteen hourly files used as a stress test.
\end{itemize}

Although the original files are compressed, Hadoop processes the decompressed text records. In the 16h stress test, Hadoop counters report more than 100 million input records and several gigabytes of logical data read. Therefore, the workload is appropriate for the required large-dataset evaluation.
...
map(offset, line):
    fields = split line into project, page, views, bytes
    if fields are malformed:
        skip line
    if views cannot be parsed or views <= 0:
        skip line

    record = PageviewRecord(project, page, inputHour, views)
